...
class ImageViewerModel: ObservableObject {
    ...
    @Published var image: NSImage? {
        didSet {
            if let image = image {
                self.originalCIImage = CIImage(data: image.tiffRepresentation!)
                applyFilter()
            }
        }
    }
    ...
    @objc private func applyFilter() {
        guard let originalCIImage = originalCIImage else { return }

        filteredCIImage = applyFilters(to: originalCIImage)
    ...
    private func updateDisplayedImage() {
        ...
        if let cgImage = context.createCGImage(filteredCIImage, from: filteredCIImage.extent) {
            let nsImage = NSImage(cgImage: cgImage, size: NSSize(width: filteredCIImage.extent.width, height: filteredCIImage.extent.height))
